...
func (mb *client) AGWriteMBBool(start int, size int, buffer []byte) (err error) {
	return mb.writeArea(s7areamk,0, start, 1, s7wlbit, buffer )
}

//implement of the interface fAGReadEB
...
func (mb *client) readArea(area int, dbNumber int, start int, amount int, wordLen int, buffer []byte) (err error) {
	...
	for totElements > 0 && err == nil {
		...
		fmt.Println("read solo request:",request)
