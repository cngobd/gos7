...
func (mb *tcpTransporter) ConnAndGetPDU() (int, error) {
	err := mb.tcpConnect()
	if err != nil {
		return 0,err
	}
	...
	return mb.pduConnect()
}
func (mb *tcpTransporter) pduConnect() (int, error) {
	...
	if err != nil {
		...
		p := response[len(response) - 2 :]
		pdu := binary.BigEndian.Uint16(p)
		return int(pdu), nil
	}
}
